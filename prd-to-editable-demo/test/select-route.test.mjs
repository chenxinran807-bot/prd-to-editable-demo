...
test('design-workspace language uses the bundled direct generation path', () => {
  assert.equal(selectRoute({ intent: '用 Open Design 做品牌级视觉探索' }).id, 'direct');
  assert.equal(selectRoute({ intent: '交给花叔 Design 做高品质界面' }).id, 'direct');
});

test('complete flow assets plus pixel fidelity select figma flow', () => {
  assert.equal(selectRoute({ intent: '按切图像素级还原', assets: ['01-flow.png', 'button.png'] }).id, 'direct');
  assert.equal(selectRoute({ intent: '按 Figma 切图像素级还原', assets: ['checkout-screen.png', 'button-slice.png'] }).id, 'direct');
});
...
test('visual design input routes to the high-fidelity specialist', () => {
  assert.equal(selectRoute({ intent: '根据截图做高保真原型', assets: ['screen.png'] }).id, 'direct');
});
...
test('a clearly named screen asset routes to high fidelity even without repeated intent words', () => {
  assert.equal(selectRoute({ intent: '做个评审原型', assets: ['checkout-screen.png'] }).id, 'direct');
});

test('a reference URL routes to the specialist that can inspect the real page', () => {
  assert.equal(selectRoute({ intent: '按这个页面做原型', url: 'https://example.com/console' }).id, 'direct');
});

test('complex product journey routes to PRD understanding specialist', () => {
  assert.equal(selectRoute({ intent: '先梳理完整用户旅程和复杂状态', assets: [] }).id, 'direct');
});

test('large PRD source routes away from the low-fidelity fallback', () => {
  const source = `${'## 模块\n需求状态处理中，删除后支持重试。\n'.repeat(5)}`;
  assert.equal(selectRoute({ intent: '', assets: [], source }).id, 'direct');
});
...
test('native and branded requests use direct professional mode; explicit Inspire remains optional', () => {
  for (const intent of ['原生移动端', '品牌高保真']) {
    const route = selectRoute({ intent, source: '# 需求', assets: [] });
    assert.equal(route.deliveryMode, 'professional');
    assert.equal(route.finalContainer, 'embedded-html');
  }
  assert.equal(selectRoute({ intent: '在 Inspire 编辑', source: '# 需求' }).id, 'inspire');
});
