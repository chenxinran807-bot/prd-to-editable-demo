...
function renderElement(element) {
  const attributes = `data-proto-key="${escapeHtml(element.key)}"${element.actionId ? ` data-action-id="${escapeHtml(element.actionId)}" data-trigger="${escapeHtml(element.trigger)}" data-visible-feedback="${escapeHtml(element.visibleFeedback)}" data-state-change="${escapeHtml(element.stateChange)}"` : ''}${element.action ? ` data-action="${escapeHtml(JSON.stringify(element.action))}"` : ''}`;
...
.semantic-region{display:flex;gap:16px}.layout-stack{flex-direction:column}.layout-grid,.layout-split{display:grid;grid-template-columns:repeat(2,minmax(0,1fr))}.layout-horizontal{flex-direction:row;overflow-x:auto}.align-start{align-items:flex-start}.align-center{align-items:center}.align-end{align-items:flex-end}.align-stretch{align-items:stretch}.scroll-region{overflow:auto;max-height:55vh}.is-sticky{position:sticky;top:0}.prominence-primary{padding:18px;background:#f4f6ff;border:1px solid #dfe3ff}.prominence-quiet{opacity:.82}
</style></head>
...
<div class="footer-status">当前页面：<span id="current-page-label"></span><span id="interaction-status" role="status" aria-live="polite"></span></div>
