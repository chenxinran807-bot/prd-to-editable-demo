...
export async function writeOutput({ outDir, html, manifest, context }) {
  ...
  const demoChecks = verifyDemo({ html, manifest });
  ...
  if (context) {
    const fidelity = context.fidelity ?? verifyFidelity({ baseline: context.baseline, model: manifest });
    const reviewRequired = fidelity.checks.some(check => check.reviewRequired);
    const status = reviewRequired ? 'review-required' : fidelity.status;
    const pageByRegion = new Map(context.ir.regions.map(region => [region.id, region.pageId]));
    const traceability = fidelity.traceability.map(item => ({
      ...item,
      sourceIds: item.kind === 'requirement'
        ? context.ir.requirements.find(requirement => requirement.id === item.id)?.sourceIds ?? [] : [],
      targetIds: item.kind === 'requirement'
        ? context.ir.requirements.find(requirement => requirement.id === item.id)?.targetIds ?? [] : [],
      pageId: item.pageId,
      elementKeys: item.elementKeys,
      regions: item.kind === 'requirement'
        ? (context.ir.requirements.find(requirement => requirement.id === item.id)?.targetIds ?? []).filter(id => pageByRegion.has(id)) : []
    }));
    Object.assign(files, {
      ...
      'traceability-matrix.json': JSON.stringify({ schemaVersion: 1, entries: traceability }, null, 2),
      'fidelity-report.md': `# Fidelity report\n\nStatus: ${status}\n\n${demoChecks.map(check => `- ${check.passed ? '[x]' : '[ ]'} ${check.name}${check.reviewRequired ? ' — subjective review required' : ''}`).join('\n')}\n`
    });
  }
